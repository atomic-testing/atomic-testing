...
If a stable `data-testid` isn't available, prefer semantic queries. `byRole` is ideal for accessibility checks while `byName` matches common form fields. Reserve raw CSS selectors only for cases that cannot be expressed with the higher level helpers.
