...
reuse the same setup for interactive debugging. You can also reuse your
Atomic drivers inside Storybook 10 play functions via
`@atomic-testing/storybook` — see
...
### 3. Avoid Direct `@testing-library/dom` Calls

Atomic Testing builds on top of `@testing-library/dom` (the same
framework-agnostic core React Testing Library itself uses) for DOM
interactions, but calling those utilities directly couples your tests
to a specific environment. For a deeper discussion of how Atomic Testing
relates to React Testing Library, see [Atomic Testing vs React Testing Library](./advanced-concepts/atomic-testing-vs-rtl.mdx).
...
as Vue, Angular or Playwright.

### 4. Use `data-testid` for Key Elements

Assign stable `data-testid` values to the elements you need to interact with. Locators built with `byDataTestId()` are both reliable and easy to read. For a full list of locator helpers see [Core Concepts](./core-concepts.mdx#locator). Avoid relying solely on CSS selectors—they tend to change as the markup evolves.
...
support. If you're testing Astryx components, check the
[Astryx driver coverage matrix](./driver-coverage/astryx-driver-coverage.md) first
to see which drivers already exist and which behaviours are DOM- or
E2E-only.
