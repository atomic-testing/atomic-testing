...
> Looking for the shorter version of this pitch — the problem, the solution, and the core principles in a few
> paragraphs? See the [Introduction](./intro.mdx). This page goes deeper: concrete before/after scenarios, the ROI
> case over time, and — just as important — when _not_ to reach for Atomic Testing.
...
> solid tool for its own environment, and for DOM testing Atomic Testing is a **sibling** of RTL, not a wrapper
> around it — both layer on top of the same underlying `@testing-library/dom` /
> `@testing-library/user-event` libraries (see
> [Atomic Testing vs RTL](../advanced-concepts/atomic-testing-vs-rtl) for the full breakdown). The pain is calling
> each framework's API **directly, per framework, with no shared layer** — so switching frameworks means rewriting
...
- **No component library** - If you're not using MUI, Radix, shadcn/ui, etc, benefit is reduced

### **✅ None of those? You're a fit — scaffold it in one command**

If the trade-offs above land on the "worth it" side for you, you don't have to wire anything up by hand. Run the
scaffolder from inside your existing project and it detects your framework, runner, and design system, then writes a
...
See the [Quick Start](./quick-start.mdx) for exactly what it detects and writes.
