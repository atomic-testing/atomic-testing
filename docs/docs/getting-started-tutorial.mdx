...
:::tip Fast path
[`create atomic-testing`](./quick-start.mdx) installs these packages and writes the Jest config for you in one command. This tutorial does it by hand so you understand each piece — reach for the CLI when you want the setup done automatically.
:::
...
The `.cjs` extension keeps this `module.exports` config as CommonJS even when your `package.json` has
`"type": "module"` (common with a Vite-scaffolded project), sidestepping the rename trap. This is why
`create atomic-testing` emits `jest.config.cjs`.
:::
...
:::info A teaching app, deliberately kept per-step
This app tests each wizard step in isolation with its own driver and its own one-part scene — that keeps every step readable on its own, which is what a tutorial needs. It's also why the [decomposing driver trees](./guides/decomposing-driver-trees.md) guide uses it as the named counter-example for driver composition: it has no page-object driver tying the steps into a flow. For a production-shaped suite composed behind a single page object, see the ticket-console app in [Example apps](./evaluate/example-apps.mdx).
:::
...
To drive those stories with the same component drivers used here — inside
Storybook's play functions and under `@storybook/addon-vitest` — see
[Testing in Storybook](./guides/testing-in-storybook.md).
...
- [API Reference](/docs/api-reference/) — the full list of locators, drivers, and interactor methods
