...
One specialization extends `ComponentDriver` for a shape many UIs share:
`ListComponentDriver`, for a repeated collection of items (e.g. a menu). A
component whose interior is a caller-supplied scene (e.g. a dialog body) needs no
specialization — every driver inherits `within(parts)`. See
[Driver Types](../core-concepts.mdx#driver-types) in Core Concepts for both, with
shipped examples.
...
can operate in any of these environments. See the [Interactor](./interactor.mdx)
