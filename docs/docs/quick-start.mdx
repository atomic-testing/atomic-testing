...
The skills and `CLAUDE.md` are on by default; pass `--no-agents` to skip them. They are the fastest path from here to a real test: open Claude Code and say **"Write tests for `<YourComponent>`"** — the skills walk the agent through decomposing the page, reusing shipped drivers, and verifying in your runner. [AI-Assisted Testing](./ai-assisted-testing.mdx) explains each skill and the prompts that trigger it.
...
<div className='card-container' style={{ display: 'flex', gap: '1rem', marginTop: '2rem', flexWrap: 'wrap' }}>
  <div className='card'>
    <div className='card__header'>
      <h3>🤖 Let Claude Write the Tests</h3>
    </div>
    <div className='card__body'>
      <p>The scaffolder just installed four Claude Code skills — say "write tests for <em>MyComponent</em>" and review the plan.</p>
      <Link to='../ai-assisted-testing' className='button button--primary'>
        Meet the Skills →
      </Link>
    </div>
  </div>
...
<div className='card'>
  ...
  <div className='card__body'>
    ...
    <Link to='../tutorial' className='button button--secondary'>
      Start Tutorial →
    </Link>
  </div>
</div>
