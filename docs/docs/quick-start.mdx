...
pnpm add react react-dom @testing-library/dom @testing-library/react @testing-library/user-event
```
...
```bash
pnpm add @atomic-testing/core @atomic-testing/vue-3 @atomic-testing/component-driver-html

# vue-3's peer dependencies — npm/yarn don't install peers automatically like pnpm can
pnpm add vue @testing-library/vue @testing-library/dom @vue/compiler-sfc @testing-library/user-event
```
...
describe('WelcomeButton', () => {
  ...
  beforeEach(() => {
    engine = createTestEngine(WelcomeButton, welcomeScene);
  });
...
```bash
pnpm add @atomic-testing/core @atomic-testing/playwright @atomic-testing/component-driver-html
