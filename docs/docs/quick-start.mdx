...
**"What about my existing tests?"** → Nothing breaks — they keep running unchanged alongside driver-based tests. Adoption is incremental: write new tests with drivers first, convert old ones only when they earn it. The full playbook is [Adoption & rollout](./evaluate/adoption-rollout.mdx).
