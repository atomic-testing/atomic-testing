...
The gate is **not yet wired into CI**: it currently reports the open accuracy findings, so enabling it would red-bar the build until [#939](https://github.com/atomic-testing/atomic-testing/issues/939) lands. Add the step below **in the same PR as the symbol fixes** so CI goes green on the first run:

```yaml
# .github/workflows/doc-ci.yml — add before "Build packages"
