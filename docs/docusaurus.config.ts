...
// TypeDoc's per-package output groups reflections into classes/interfaces/
// functions/type-aliases/variables subfolders, and Docusaurus's autogenerated
// sidebar mirrors that as a nested "Classes" category a reader has to expand
// to reach the driver they came for. Dissolve those kind-folder categories
// wherever they appear in the generated tree, inlining their children as
// direct siblings instead — a docs/component-driver-mui-v9 package shows its
// ~40 driver classes and handful of supporting types as one flat list, not
// classes buried one click deeper than everything else.
// Lowercase: Docusaurus's autogenerated category generator uses these
// TypeDoc output folder names (classes/, interfaces/, etc.) verbatim as the
// category label when there's no _category_.json — it does NOT title-case
// them, confirmed against the actual generated sidebar HTML.
const TYPEDOC_KIND_CATEGORY_LABELS = new Set([
  'classes',
  'interfaces',
  'type aliases',
  'type-aliases',
...
function flattenTypedocKindCategories(items: any[]): any[] {
  const out: any[] = [];
  for (const item of items) {
    if (item.type === 'category') {
      const flattenedChildren = flattenTypedocKindCategories(item.items);
      if (typeof item.label === 'string' && TYPEDOC_KIND_CATEGORY_LABELS.has(item.label.toLowerCase())) {
        out.push(...flattenedChildren);
      } else {
        out.push({ ...item, items: flattenedChildren });
      }
    } else {
      out.push(item);
    }
      ...
      /** @type {import('@docusaurus/preset-classic').Options} */
      {
        docs: {
          ...
          sidebarItemsGenerator: async ({ defaultSidebarItemsGenerator, ...args }) => {
            ...
            return flattenTypedocKindCategories(items);
          },
