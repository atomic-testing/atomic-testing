...
| `drag`            | Synthesize `mousedown` → `mousemove(+delta)` → `mouseup` from the caller-supplied `delta` via `fireEvent` (deltas are caller-given, so no layout needed for the event payload). Behavioral outcome still E2E-only.                                    | Manual `mouse.move/down/move/up` from `boundingBox()` center + `delta` as one uninterrupted gesture, not composed from the discrete hover-then-act helpers. (At decision time this also **avoided the `mouse.move(0,0)` reset** `mouseMove` then performed after hovering; that reset was later removed — see Options §5 — but `drag` still owns its own continuous sequence.) |
...
5. **`drag` reusing the existing `mouseMove`/`mouseDown` building blocks in Playwright.**
   ...
   **Update (#1057):** `mouseMove` no longer performs the `page.mouse.move(0,0)` reset (it
   now leaves the pointer hovered, matching `DOMInteractor`), so that specific premise is
   gone — but the decision stands on the "one uninterrupted gesture" reason above, which is
   engine- and helper-independent, so `drag`/`dragTo` still own their sequence.
