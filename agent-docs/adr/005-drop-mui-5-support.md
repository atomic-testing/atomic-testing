...
- Their `check:type` scripts are removed so the recursive `pnpm -r check:type`
  ...
  means their `dist` (and thus `.d.ts`) is no longer built, and
  `mui-x-v5` imports `ButtonDriver` from `mui-v5` — type-checking the frozen
  packages would fail to resolve that cross-package type. v5 is no longer
  type-checked, consistent with being frozen.
