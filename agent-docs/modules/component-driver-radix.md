...
Barrel: [component-driver-radix-v1/src/index.ts](../../packages/component-driver-radix-v1/src/index.ts) — the canonical export list. Wave 0 shipped the proof-of-life `SeparatorDriver` (`getOrientation` via `data-orientation`; `isDecorative` via `role="none"`). Wave 1 (#1003) adds `DialogDriver`, `DropdownMenuDriver`, `PopoverDriver`, `SelectDriver`, and a shared internal `MenuItemDriver` (mirrors `component-driver-mui-v7`'s reuse of one item driver across `MenuDriver`/`SelectDriver`); driver waves 3–5 extend further. Errors follow the mui-v7 `src/errors/<Name>Error.ts` pattern; Wave 1 added `MenuItemNotFoundError`, shared by `DropdownMenuDriver.selectByLabel` and `SelectDriver.selectByLabel` for the same reason MUI shares it.
