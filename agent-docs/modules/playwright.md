...
`PlaywrightInteractor(page)` resolves every `PartLocator` through the private `resolveLocator(locator)` seam (#923), which returns a Playwright `Locator` every primitive then calls `.<action>()` on. For an ordinary CSS-only locator this is exactly `locatorUtil.toCssSelector(locator, this)` → `page.locator(css)`, unchanged from before; a `findByRole` locator (an `AccessibleRoleLocator` segment, detected via `locatorUtil.splitAtAccessibleRoleLocator` before `toCssSelector` is reached) instead resolves its preceding scope recursively and calls Playwright's own accname-aware `Locator.getByRole`/`Page.getByRole` — see [ADR-018](../adr/018-findbyrole-accessible-role-locator.md) ([PlaywrightInteractor.ts](../../packages/playwright/src/PlaywrightInteractor.ts)). Notable points vs the DOM implementation:
...
- **`mouseMove`** hovers and leaves the pointer over the target (no reset), so hover-dependent state survives — matching `DOMInteractor.mouseMove` (#1057) ([PlaywrightInteractor.ts](../../packages/playwright/src/PlaywrightInteractor.ts)).
...
- **`isReadonly`** is inferred from the `readonly` attribute; **`isChecked`/`isDisabled`** evaluate core's `elementStateUtil` predicate in-page rather than delegating to Playwright's own primitives, which retarget through labels and carry their own notion of which elements can be disabled — the same shared-predicate technique `isVisible` uses, so the two engines agree by construction ([PlaywrightInteractor.ts](../../packages/playwright/src/PlaywrightInteractor.ts)).
- **Layout/geometry primitives are Playwright-native**: `getBoundingRect`/`drag` use `boundingBox()` (which returns `null` rather than auto-waiting, so they throw `ElementNotFoundError` synchronously instead of via the `runMutation` timeout path); `drag` builds the gesture from `getBoundingRect`'s center via `page.mouse` as one uninterrupted `move/down/move/up` sequence; `scrollIntoView` uses `scrollIntoViewIfNeeded`, `scrollBy` evaluates `el.scrollBy` (deterministic cross-engine, vs `mouse.wheel`); `dragTo` uses native `Locator.dragTo`; `setInputFiles` reads paths from disk; `contextMenu` is a right-button `click`; `pressKey` builds a `Modifier+Key` chord (Shift + a printable key does NOT case-fold `event.key` — verified to match jsdom, see #924).
- **`drag`/`dragTo` also drive native HTML5 drag-and-drop** (#922) — verified empirically that Playwright's real, low-level pointer gesture (`Locator.dragTo`, or `drag`'s manual `page.mouse` sequence) makes the browser's own native DnD recognition fire the full `dragstart`→`dragenter`/`dragover`→`drop`→`dragend` sequence with a real, populated `DataTransfer` on a `draggable` element — no extra event dispatch needed, unlike jsdom (see `dom-core.md`'s `FakeDataTransfer`).
