...
**Critical jsdom/jest detail:** because `@astryxdesign/core` is ESM-only and the shared `jest.config.base.js` does not transform `node_modules`, the test package's [jest.config.js](../../package-tests/component-driver-astryx-test/jest.config.js) adds a `'^.+\.(js|mjs)$': '@swc/jest'` transform rule **and** `transformIgnorePatterns: ['/node_modules/(?!.*(?:@astryxdesign|@stylexjs|intl-messageformat|@formatjs))']` (the `.*` lookahead is pnpm-real-path safe). The `intl-messageformat`/`@formatjs/*` exception is for Astryx 0.1.7's component translation (`useTranslator`), which pulls those ESM-only packages in transitively. With this, the real Astryx component renders under jsdom — no component mock is needed; only CSS imports are mocked. Prefer the subpath import `@astryxdesign/core/Button` (smaller transform surface than the barrel).

**jsdom web-platform shims** ([jest.setup.ts](../../package-tests/component-driver-astryx-test/jest.setup.ts), wired via `setupFiles`): jsdom lacks several APIs Astryx overlays use. The setup polyfills the **Popover API** (`showPopover`/`hidePopover`/`togglePopover` as no-ops), the native **`<dialog>` modal methods** (`show`/`showModal`/`close`, reflected onto the `open` attribute the driver reads — `showModal` otherwise throws and tears down the dialog subtree), **`window.matchMedia`** (Toast → `useTheme`), **`window.scrollTo`** (Dialog scroll-lock), a **`CSS.escape`** polyfill (Astryx 0.1.9's `Dialog` auto-labelling effect calls it, and jsdom exposes no global `CSS`), and an inert **`HTMLCanvasElement.getContext`** (silences a console warning from `Spinner`'s canvas ring, which already no-ops when `getContext` returns falsy). True overlay visibility/animation is not modelled — that is the Playwright run's job.
