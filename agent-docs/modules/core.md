...
| `collectionUtil`, `dateUtil`, `escapeUtil`, `locatorUtil`, `timingUtil`, `interactorUtil`, `visibilityUtil`, `elementStateUtil`         | namespaces              | [index.ts#L17-L30](../../packages/core/src/index.ts#L17-L30)                                                                           |
...
| `findByRole(role, name?)`       | NOT CSS — computed accessible name, via `AccessibleRoleLocator` | [findByRole.ts](../../packages/core/src/locators/findByRole.ts)               |

Composition lives in `locatorUtil` ([utils/locatorUtil.ts](../../packages/core/src/utils/locatorUtil.ts)): `append(...locators)` chains while respecting `Root` boundaries; `toCssSelector(locator, interactor)` resolves to a runtime selector (awaiting linked-locator resolution); `overrideLocatorRelativePosition(...)` rewrites a locator's relative position. Disambiguate two same-role elements by VERBATIM `aria-label` with the same-element compound `locatorUtil.and(byRole(role), byAriaLabel(name))` ([locatorUtil.and](../../packages/core/src/utils/locatorUtil.ts) — supersedes the older `append(byRole(role), byAriaLabel(name, 'Same'))`). For the COMPUTED accessible name (`aria-labelledby` / `<label>` / text content — not CSS-expressible), use `findByRole(role, name)` instead (#923, [ADR-018](../adr/018-findbyrole-accessible-role-locator.md)): it builds an `AccessibleRoleLocator` (a `CssLocator` subclass with no real CSS selector) that `locatorUtil.splitAtAccessibleRoleLocator` detects before `toCssSelector` is reached, routing resolution to each interactor's own accname engine instead. It must be the LAST locator in a chain; composing it via `and()` throws (its `complexity` is `'accessibleRole'`, not `'primitive'`).

## Utilities

| Namespace          | Notable members                                                                                                                                                                      | File                                                                     |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| `timingUtil`       | `wait(ms)`, `waitUntil(option)` (probe loop; escalating cadence by default or via `probeIntervals`, even `timeoutMs/probeCount` grid when `probeCount` is supplied)                  | [timingUtil.ts](../../packages/core/src/utils/timingUtil.ts)             |
...
| `elementStateUtil` | `isElementChecked`, `isElementDisabled` — the checked/disabled policy both engines run, so jsdom and Playwright cannot drift. Self-contained: Playwright serializes it into the page | [elementStateUtil.ts](../../packages/core/src/utils/elementStateUtil.ts) |
