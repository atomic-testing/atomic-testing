...
| **ListComponentDriver**  | A `ComponentDriver` for repeated, indefinite-length item collections; addresses items by locator MATCH index (`Interactor.getMatchLocator`).                                                    | [ListComponentDriver.ts](../packages/core/src/drivers/ListComponentDriver.ts#L16)                                                                    |
...
- **List items are addressed by match index.** `ListComponentDriver` / `listHelper` locate the *n*th item through `Interactor.getMatchLocator(itemLocator, n)` — the same match-order reckoning `getItemCount()` counts by, so the two agree for any list shape, including items interleaved with same-tag non-items (#1054). The address is normally a live `:nth-child(k)` compounded onto the item locator; see `matchAddressUtil` for the absolute-path fallback when a match set spans several parents ([listHelper.ts](../packages/core/src/drivers/listHelper.ts), [matchAddressUtil.ts](../packages/core/src/utils/matchAddressUtil.ts)). Items nested inside wrapper elements still need `childListHelper`'s `groupSelector` walk.
- **Default wait** is `{ condition: 'attached', timeoutMs: 30000, debug: false }` ([WaitForOption.ts#L28-L32](../packages/core/src/drivers/WaitForOption.ts#L28-L32)). `waitUntilVisible()` derives its default timeout from the same `defaultWaitForOption.timeoutMs`, so both waits share the 30s tolerance (#1057) ([ComponentDriver.ts](../packages/core/src/drivers/ComponentDriver.ts)).
