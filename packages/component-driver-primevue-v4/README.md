...
| `DialogDriver`                                | `Dialog`                                    | Interior reached with `scope(parts)`; portal re-root on `role="dialog"`; title via `aria-labelledby`; close via header button or Escape. `{ selfAnchored: true }` opts into `appendTo="self"` — see "Teleported overlays" below.                                                                                                                                                     |
