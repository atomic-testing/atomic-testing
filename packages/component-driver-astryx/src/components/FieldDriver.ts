...
export class FieldDriver extends ComponentDriver<{}> {
  ...
  async isRequired(): Promise<boolean> {
    return (await this.getLabel())?.includes('Required') ?? false;
  }

  /** Whether the field is marked optional (Astryx appends an "Optional" label marker). */
  async isOptional(): Promise<boolean> {
    return (await this.getLabel())?.includes('Optional') ?? false;
  }
