...
export class TableDriver extends ComponentDriver {
  ...
  private row(index: number): PartLocator {
    return locatorUtil.append(this.locator, byCssSelector(`tbody tr:nth-child(${index + 1})`));
  }
  ...
  async getColumnHeaders(): Promise<readonly string[]> {
    ...
    for (let k = 1; await this.interactor.exists(this.headerCell(k)); k++) {
  ...
  async getRowCellTexts(index: number): Promise<readonly string[]> {
    const texts: string[] = [];
    for (let k = 1; await this.interactor.exists(this.bodyCell(index, k)); k++) {
