...
| `DialogDriver`                              | `DialogRoot`/`DialogContent`                                               | Interior reached with `scope(parts)`; portal re-root (`overriddenParentLocator`/`overrideLocatorRelativePosition` → `byRole('dialog', 'Root')`/`'Same'`) to the document-level `[role="dialog"]`. `getTitle`, `closeByEscape`, `waitForOpen`/`waitForClose`, `isOpen`.                                                     |
