...
The local SCM may be **Git, Sapling (`sl`), or any git-compatible system** — this repo currently uses **Sapling**. Two traps to avoid: the environment's `Is a git repository: false` probe is **unreliable** (Sapling reports this; the repo _is_ versioned — detect, don't trust it), and **GitHub operations are SCM-agnostic** — raise issues/PRs/reviews with the authenticated `gh` CLI regardless of the local SCM (pass `-R atomic-testing/atomic-testing`, since `gh` can't infer the repo from a Sapling checkout). A SessionStart hook surfaces the active SCM each session. Canonical detection + command map: [`.claude/scm.md`](.claude/scm.md).
