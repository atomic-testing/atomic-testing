# @atomic-testing/angular-21-test

Validation fixture for `@atomic-testing/angular-21` (and therefore
...
This is a sibling of `angular-20-test` pinned to Angular 21 instead — see that
...
only the pinned `@angular/*`/`@atomic-testing/angular-21` versions differ.

## Run

```bash
# Build the adapter first — tests resolve @atomic-testing/* to dist (stale-dist trap)
pnpm --filter "@atomic-testing/angular-21..." build
