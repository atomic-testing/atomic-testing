...
import { byDataTestId, byRole, IExampleUnit, ScenePart } from '@atomic-testing/core';
...
export const buttonExampleScenePart = {
  ...
  // step-2 name-aware byRole overload, the role/name-first path Astryx needs.
  openButton: {
...
export const buttonExampleTestSuite: TestSuiteInfo<typeof buttonExample.scene> = {
  ...
  tests: (getTestEngine, { describe, test, beforeEach, afterEach, assertEqual, assertTrue, assertFalse }) => {
    describe(`${buttonExample.title}`, () => {
      ...
      // aria-label, proving byRole('button', { name }) disambiguation.
      test(`byRole name disambiguates two same-role buttons`, async () => {
